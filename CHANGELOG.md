...
### Added
- **4.D.8**: UI AI-ассистент — SSE streaming + AIHintButton + ContextualSuggestionCard
  - SSE streaming чат на странице `/blocks/7`:
    - Token-by-token отображение ответа AI в реальном времени
    - Кнопка остановки генерации (abort controller)
    - Fallback на обычный POST при ошибке SSE
    - Курсор-анимация при streaming
  - `AIHintButton` — компонент для встраивания в другие блоки:
    - Popover с AI-подсказками для конкретного блока (1-8)
    - Приоритеты (high/medium/low) и типы действий (generate/validate/fix/review)
    - Автозагрузка при открытии
  - `ContextualSuggestionCard` — плавающая карточка с AI-подсказками:
    - Compact виджет с максимум 3 подсказками
    - Dismiss с сохранением состояния
    - Автозагрузка при монтировании
  - Вкладка «История» — загружаемая из сервера история чата:
    - Группировка по датам
    - Пагинация (load more)
    - Кнопка очистки
  - Индикатор текущего блока пайплайна в header

### Changed
- Версия обновлена с 0.36.0 до 0.36.1
- Страница `/blocks/7` полностью переработана: SSE streaming, 4 вкладки, abort control
- Добавлен компонент shadcn/ui `popover.tsx`
